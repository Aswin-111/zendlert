...
const AdminController = {
    ...
    try {
      ...
      const alerts = await prisma.alerts.findMany({
        where: {
          organization_id,
          ...(alert_type && {
            message: { contains: alert_type, mode: "insensitive" },
          }),
        },
        include: {
      ...
      const responseRate = totalAlerts
        ? (acknowledgedCount / totalAlerts) * 100
        : 0;

      // Weather alert detection (example logic)
      const weatherAlert = alerts.some((alert) =>
        alert.message.toLowerCase().includes("weather"),
      );
  ...
  getBuildingAlerts: async (req, res) => {
    try {
      ...
      const recentAlerts = alerts.filter(
        (a) =>
          a.created_at <= now && (!a.scheduled_time || a.scheduled_time <= now),
      );
      ...
      const emergencyContact = area.site.organization.main_contact_name
        ? {
  ...
  getAllBuildingAlerts: async (req, res) => {
    try {
      ...
      const now = new Date();
      const recentAlerts = alerts.filter(
        (alert) => alert.start_time && alert.start_time <= now,
      );
      const upcomingAlerts = alerts.filter(
        (alert) => alert.scheduled_time && alert.scheduled_time > now,
      );
      const scheduledAlerts = alerts.filter(
        (alert) => !alert.start_time && alert.scheduled_time,
      );
    ...
    try {
      ...
      if (user_type === UserTypes.employee) {
        ...
        if (!contracting_company_id) {
          return res
            .status(400)
            .json({ message: "Contracting company is required." });
        }
  ...
  toggleEmployeeStatus: async (req, res) => {
    try {
      ...
      if (!user_id || !organization_id || !status) {
        return res.status(400).json({
          message:
            "user_id, organization_id, and status are required query parameters.",
        });
      }
      ...
      if (status !== "activate" && status !== "deactivate") {
        return res.status(400).json({
          message: "Invalid status value. Must be 'activate' or 'deactivate'.",
        });
      }
  ...
  createAlert: async (req, res) => {
    try {
      ...
      const alert_name = await prisma.emergency_Types.findUnique({
        where: { id: parsed.emergency_type_id },
        select: { name: true },
      });
            ...
            (async () => {
              try {
                ...
                if (
                  ...
                  console.warn(
                    `Removed invalid FCM token for user ${i.user_id}`,
                  );
                }
              }
            })();
  ...
  reportNotification: async (req, res) => {
    try {
      ...
      if (lastRecepient.delivery_status === DeliveryStatus.DELIVERED) {
        return res
          .status(400)
          .json({ message: "Notification already reported" });
      }
      ...
      return res
        .status(200)
        .json({ message: "Notification reported successfully" });
    } catch (error) {
  ...
  createContractingCompany: async (req, res) => {
    try {
      ...
      const { organization_id, name, contact_email, phone, address } =
        parsed.data;
  ...
  editContractingCompany: async (req, res) => {
    try {
      ...
      if (contact_email)
        data.contact_email = contact_email.trim().toLowerCase();
      ...
      return res
        .status(200)
        .json({ message: "Company updated", company: updated });
    } catch (err) {
      if (err?.code === "P2025") {
  ...
  getContractingActiveEmployees: async (req, res) => {
    try {
      ...
      if (!organization_id || !company_id) {
        return res
          .status(400)
          .json({ message: "organization_id and company_id are required" });
      }
      ...
      if (!company) {
        return res.status(404).json({
          message: "Contracting company not found for the given organization",
        });
      }
  ...
  getOrganizationOverview: async (req, res) => {
    try {
      ...
      const [permanent_employees, contractors, pending_verifications] =
        await Promise.all([
  ...
  getEmployees: async (req, res) => {
    try {
      ...
      const where = {
        ...
        ...(roles ? { role_id: { in: roles.split(",") } } : {}),

        // Optional site filter
        ...(sites ? { site_id: { in: sites.split(",") } } : {}),
      ...
      const role_details = await prisma.roles.findMany({
      ...
      const results = users.map((user) => {
        ...
        return {
          ...
          is_employee,
          company_name: !is_employee
            ? user.contractors?.[0]?.contracting_company?.name || null
            : null,
      ...
      if (user_type && user.user_type !== user_type) {
        if (user_type === "employee") {
          ...
          if (!company_id)
            return res
              .status(400)
              .json({ message: "company_id is required for contractors" });
  ...
  deleteContractingCompany: async (req, res) => {
    try {
      ...
      if (!company_id) {
        return res
          .status(400)
          .json({ message: "contracting_company_id is required" });
      }
      const contracting_company = await prisma.contracting_Companies.findUnique(
        { where: { id: company_id, organization_id } },
      );
      console.log(contracting_company);
      if (!contracting_company) {
        return res
          .status(400)
          .json({ message: "Contracting company not found" });
      }
      ...
      return res
        .status(200)
        .json({ message: "Contracting company deleted successfully" });
    } catch (error) {
  ...
  getSitesCards: async (req, res) => {
    try {
      ...
      const [total_sites, active_sites, total_areas, current_occupancy] =
        await Promise.all([
          prisma.sites.count({
            where: { organization_id },
          }),
  ...
  searchSites: async (req, res) => {
    try {
      ...
      const pageSizeNum = Math.min(
        Math.max(parseInt(page_size, 10) || 10, 1),
        100,
      );
      const skip = (pageNum - 1) * pageSizeNum;
      const take = pageSizeNum;
  ...
  deleteArea: async (req, res) => {
    try {
      ...
      if (!site_id || !area_id) {
        return res
          .status(400)
          .json({ error: "site_id and area_id are required" });
      }
      ...
      if (!area) {
        return res
          .status(404)
          .json({ error: "Area not found for the given site" });
      }
  ...
  sitePopupOverview: async (req, res) => {
    try {
      ...
      if (responses.length > 0) {
        const diffs = responses.map(
          (r) => (r.response_updated_at - r.created_at) / 1000, // seconds
        );
  ...
  getSitePopupAlerts: async (req, res) => {
    try {
      ...
      const response = alerts.map((alert) => {
        // counts
        const safeCount = alert.Notification_Recipients.filter(
          (r) => r.response === "SAFE",
        ).length;
        const unsafeCount = alert.Notification_Recipients.filter(
          (r) => r.response === "NOT_SAFE",
        ).length;
        const notRespondedCount = alert.Notification_Recipients.filter(
          (r) => !r.response,
        ).length;
    ...
    try {
      ...
      const data = alerts.map((a) => {
        ...
        const notResponded = Math.max(
          0,
          recipients.length - (safe + needHelp + emergency),
        );
  ...
  getIndividualAlertDetails: async (req, res) => {
    try {
      const { alert_id, organization_id } = req.query;
      ...
      if (
        organization_id &&
        String(alert.organization_id) !== String(organization_id)
      ) {
        return res.status(403).json({
          message: "Forbidden: alert does not belong to this organization.",
        });
      }
        ...
        select: {
          ...
          user: {
            select: {
              first_name: true,
              last_name: true,
              user_type: true, // employee | contractor
            },
          },
        },
      });
      ...
      const alert_time =
        alert.start_time ?? alert.scheduled_time ?? alert.created_at;
      ...
      const users = recipients.map((r) => ({
        ...
        alert_time,
        status: r.response ?? "not_responded",
        delivery_status: r.delivery_status,
        user_type: r.user?.user_type ?? null,
        alert_status: alert.status,
        user_report_time: r.response_updated_at ?? r.acknowledged_at ?? null
      }));
      ...
      console.error("getIndividualAlertDetails error:", error);
      return res.status(500).json({
        message: "Server error",
        error: error.message,
      });
  ...
  getSitePerformance: async (req, res) => {
    try {
      ...
      for (const { site_id, alert_id } of alertLinks) {
        if (!siteToAlertIds.has(site_id))
          siteToAlertIds.set(site_id, new Set());
      ...
      for (const r of responders) {
        ...
        if (!siteId) continue;
        if (!siteToResponderSet.has(siteId))
          siteToResponderSet.set(siteId, new Set());
      ...
      const site_performance = sitesPage.map((site) => {
        ...
        const performancePct =
          total_people > 0
            ? ((uniqueResponders / total_people) * 100).toFixed(2)
            : "0.00";
  ...
  getAlertDistribution: async (req, res) => {
    try {
      ...
      const countMap = new Map(
        grouped.map((g) => [g.emergency_type_id, g._count._all]),
      );
  ...
  getGeneralSettings: async (req, res) => {
    try {
      const organization_id =
        req.user?.organization_id || req.query.organization_id;
      ...
      const street = primarySite
        ? [primarySite.address_line_1, primarySite.address_line_2]
            .filter(Boolean)
            .join(", ")
        : "";
  ...
  updateGeneralSettings: async (req, res) => {
    try {
      const organization_id =
        req.user?.organization_id || req.body.organization_id;
      ...
      await prisma.organizations.update({
        ...
        data: {
          ...
          ...(primary_contact_name !== undefined
            ? { main_contact_name: primary_contact_name }
            : {}),
          ...
          await prisma.sites.update({
            ...
            data: {
              ...
              ...(organization_address.street_address_line_2 !== undefined
                ...
                : {}),
              ...(organization_address.city !== undefined
                ? { city: organization_address.city }
                : {}),
              ...(organization_address.state !== undefined
                ? { state: organization_address.state }
                : {}),
              ...(organization_address.zip !== undefined
                ? { zip_code: organization_address.zip }
                : {}),
            },
          });
        }

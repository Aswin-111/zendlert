    ...
    try {
      ...
      const formattedPlans = plans.map((plan) => {
        ...
        if (plan.site_limit > 0) {
          uiFeatures.push(
            plan.site_limit >= 9999
              ? "Unlimited Sites"
              : `Up to ${plan.site_limit} Sites`,
          );
        }
        if (plan.area_limit > 0) {
          uiFeatures.push(
            plan.area_limit >= 9999
              ? "Unlimited Areas"
              : `Up to ${plan.area_limit} Areas`,
          );
        }
        if (plan.alert_limit > 0) {
          uiFeatures.push(
            plan.alert_limit >= 9999
              ? "Unlimited Alerts"
              : `${plan.alert_limit} Alerts / Month`,
          );
        }

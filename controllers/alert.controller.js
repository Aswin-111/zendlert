...
const startNotificationWorker = () => {
    ...
    const sendSmsBatch = async (recipients) => {
        console.log(`[SMS WORKER] Placeholder: Simulating sending of ${recipients.length} SMS messages.`);
    ...
    const worker = new Worker('notificationQueue', async (job) => {
        ...
        try {
            ...
            if (!alert) throw new Error(`Alert ${alert_id} not found during update.`);
            ...
            await prisma.notification_Recipients.createMany({
                data: recipients.map(user => ({ alert_id: alert.id, user_id: user.user_id })),
                skipDuplicates: true
            });
            ...
            if (recipientsWithFcmTokens.length > 0) {
                ...
                const message = {
                    notification: { title: `ALERT: ${alert.severity.toUpperCase()}`, body: alert.message },
                    tokens,
                    data: { alert_id: alert.id }
                };
                const response = await admin.messaging().sendMulticast(message);
                logger.info(`[FCM WORKER] Successfully sent ${response.successCount} push notifications for alert ${alert.id}.`);
                if (response.failureCount > 0) {
                    logger.warn(`[FCM WORKER] Failed to send ${response.failureCount} push notifications for alert ${alert.id}.`);
                }
            }
            ...
            if (send_sms) {
                ...
                if (recipientsWithPhone.length > 0) {
                    await sendSmsBatch(recipientsWithPhone.map(r => ({ phone_number: r.phone_number, message: alert.message })));
                }
            }
        } catch (error) {
            logger.error(`[WORKER] Job ${job.id} failed for alert ${alert_id}:`, { error });
            throw error; // Let BullMQ handle the retry according to the backoff strategy
        }
    }, { connection: redisConnection });

    worker.on('failed', (job, err) => logger.error(`Job ${job.id} has failed with ${err.message}`));
...
const createAlertSchema = z.object({
    ...
    timing_details: z.object({
        timing: z.enum(["send_now", "scheduled"]),
        scheduled_time: z.string().optional(),
    }).refine(data => data.timing !== 'scheduled' || data.scheduled_time, { message: "scheduled_time is required for scheduled alerts." }),
    selected_area_details: z.object({
        site_selections: z.array(z.object({
...
const AlertController = {
    ...
    getAlertDashboard: async (req, res) => {
        try {
            ...
            const deliveryAverage = totalDeliveries ? (deliveredCount / totalDeliveries) * 100 : 0;
    ...
    getRecipientCountsByArea: async (req, res) => {
        try {
            ...
            if (!organization_id || !area_ids) {
                return res.status(400).json({ message: "organization_id and area_ids are required in the request body." });
            }

            // UPDATED: Validate that area_ids is a non-empty array
            if (!Array.isArray(area_ids) || area_ids.length === 0) {
                return res.status(400).json({ message: "area_ids must be a non-empty array." });
            }
            ...
            if (validAreas.length !== areaIdArray.length) {
                ...
                return res.status(400).json({
                    message: "One or more area IDs are invalid or do not belong to the specified organization.",
    ...
    createAlert: async (req, res) => {
        try {
            ...
            if (!validation.success) {
                ...
                return res.status(400).json({ message: "Invalid input", errors: validation.error.flatten() });
            }
            const { user_id, organization_id, alert_type, severity_level, alert_message, send_sms, response_required, timing_details, selected_area_details } = validation.data;
            ...
            const [user, organization, emergencyType] = await Promise.all([
                ...
                prisma.emergency_Types.findFirst({ where: { name: alert_type, organization_id } })
            ]);

            if (!user) return res.status(404).json({ message: "User not found." });
            if (!organization) return res.status(404).json({ message: "Organization not found." });
            if (user.organization_id !== organization_id) return res.status(403).json({ message: "Forbidden: User does not belong to the organization." });
            if (!emergencyType) return res.status(404).json({ message: `Alert type '${alert_type}' not found for this organization.` });

            const incomingSiteIds = selected_area_details.site_selections.map(s => s.site_id);
            const validSitesCount = await prisma.sites.count({ where: { id: { in: incomingSiteIds }, organization_id } });
            if (validSitesCount !== incomingSiteIds.length) return res.status(400).json({ message: "One or more site IDs are invalid or do not belong to the organization." });
            ...
            for (const selection of selected_area_details.site_selections) {
                const areas = await prisma.areas.findMany({
                    where: {
                        ...
                        id: selection.area_ids.length > 0 ? { in: selection.area_ids } : undefined
                    },
                    select: { id: true }
                });
                if (selection.area_ids.length > 0 && areas.length !== selection.area_ids.length) return res.status(400).json({ message: `One or more area IDs are invalid for site '${selection.site_id}'.` });
                areas.forEach(area => finalAreaIds.add(area.id));
            }
            const finalAreaIdsArray = Array.from(finalAreaIds);
            if (finalAreaIdsArray.length === 0) return res.status(400).json({ message: "No areas found to send alert to." });
            ...
            const scheduled_time = timing === 'scheduled' ? new Date(scheduledTimeStr) : null;
            if (status === AlertStatus.scheduled && scheduled_time <= new Date()) return res.status(400).json({ message: "Scheduled time must be in the future." });

            const newAlert = await prisma.$transaction(async (tx) => {
                const createdAlert = await tx.alerts.create({
                    data: { user_id, organization_id, emergency_type_id: emergencyType.id, severity: severity_level, message: alert_message, response_required, status, start_time, scheduled_time }
                });
                await tx.alert_Sites.createMany({ data: incomingSiteIds.map(site_id => ({ alert_id: createdAlert.id, site_id })) });
                await tx.alert_Areas.createMany({ data: finalAreaIdsArray.map(area_id => ({ alert_id: createdAlert.id, area_id })) });
                return createdAlert;
            });

            if (newAlert.status === 'active') {
                await notificationQueue.add('send-alert-notifications', { alert_id: newAlert.id, send_sms });
                ...
                if (delay > 0) {
                    await notificationQueue.add('send-alert-notifications', { alert_id: newAlert.id, send_sms }, { delay });
                    logger.info(`[API] Job scheduled in queue for alert ${newAlert.id} with a delay of ${Math.round(delay / 1000)}s`);
                }
            }
    ...
    resolveAlert: async (req, res) => {
        try {
            ...
            if (!organization_id || !alert_id || !message) {
                return res.status(400).json({
                    message: "organization_id, alert_id, and a resolution message are required in the request body."
                });
            }
            ...
            if (alert.organization_id !== organization_id) {
                return res.status(403).json({ message: "Forbidden: You do not have permission to resolve this alert." });
            }

            // STATE CHECK: Ensure the alert is currently active.
            if (alert.status !== 'active') { // Using direct string comparison for clarity
                return res.status(409).json({ // 409 Conflict is appropriate for incorrect state
            ...
            return res.status(200).json({ message: "Alert has been successfully resolved." });
